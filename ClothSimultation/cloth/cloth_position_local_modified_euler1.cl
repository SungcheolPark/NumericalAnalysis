...
void cloth_position_2nd(
	...
	if (get_local_id(0) == 0 && get_local_id(1) == 0) { // ¿ÞÂÊ À§ ±ÍÅüÀÌ
		...
		local_data[get_local_id(0) + (get_local_size(0) + 2) * (get_local_id(1) + 2)] = pos_in[(get_00global_id(0) - 1) + get_global_size(0)* (get_global_id(1) + 1)];
	}
	...
	pos_out[global_idx] = local_data[local_idx] + vel_in[global_idx] * DeltaT + (float4)(a * DeltaT * DeltaT / 2, 0.0);  // p(i+1) 
	...
	force += -DampingConst * (v + a * DeltaT);

	float3 a_t = force * ParticleInvMass;

	vel_out[global_idx] = vel_in[global_idx] + (float4)(a * DeltaT / 2, 0.0) + (float4)(a_t * DeltaT / 2, 0.0);   
